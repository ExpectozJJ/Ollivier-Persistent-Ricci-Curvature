...
path1 = "ORC PLB IDM 2016/train_graphs/"
path2 = "ORC PLB IDM 2016/test_graphs/"
path3 = "ORC PLB IDM 2016/Complexes/"
...
"""
if os.path.isfile("ORC PLB IDM 2016/train_list.npz") and os.path.isfile("ORC PLB IDM 2016/test_list.npz"):
    ...
    [training_set, test_set, refined_set] = listprep("ORC PLB IDM 2016/INDEX_refined_data.2016", "ORC PLB IDM 2016/INDEX_core_data.2016.lst")
    np.savez("ORC PLB IDM 2016/train_list.npz", training_set)
    np.savez("ORC PLB IDM 2016/test_list.npz", test_set)
    list2 = np.load("ORC PLB IDM 2016/train_list.npz", allow_pickle=True)["arr_0"]
    test_list = np.load("ORC PLB IDM 2016/test_list.npz", allow_pickle=True)["arr_0"]
"""
...
def create_test(threshold, cutoff, num_of_gaps):
    ...
    for i in range(a, b+1):
        ...
        np.save("ORC PLB IDM 2016/test_graphs/"+pdbid+"_"+str(cutoff)+"_"+str(threshold)+".npy", X_graphs)
    ...
    for i in range(a, b+1):
        ...
        for i in range(0, len(pro_coords)):
            for j in range(0, len(lig_coords)):
                ...
                X_graphs.append(temp_feature)
        #feature_x.append(X_feature)
        np.save("ORC PLB IDM 2016/train_graphs/"+pdbid+"_"+str(cutoff)+"_"+str(threshold)+".npy", X_graphs)
...
def convertall():
    ...
    for i in range(len(list2)):
        convertpdb("ORC PLB IDM 2016/PDB/"+list2[i]+"_protein.pdb")
        convertmol2("ORC PLB IDM 2016/MOL/"+list2[i]+"_ligand.mol2")
        create_complex(list2[i], 10)
    for i in range(len(test_list)):
        convertpdb("ORC PLB IDM 2016/PDB/"+test_list[i]+"_protein.pdb")
        convertmol2("ORC PLB IDM 2016/MOL/"+test_list[i]+"_ligand.mol2")
...
for num_of_bins in range(10, 101, 10):
    for fp in range(6, 16):
        train_set = np.load("ORC PLB IDM 2016/train_set_"+str(num_of_bins)+"_"+str(fp)+".npy", allow_pickle=True)
        temp = np.load("ORC PLB IDM 2016/train_graphs/1w8l_10_15.npy", allow_pickle=True)
        ...
        np.save("ORC PLB IDM 2016/train_set_"+str(num_of_bins)+"_"+str(fp)+".npy", X_train)
"""
