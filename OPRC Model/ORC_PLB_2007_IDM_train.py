...
path1 = "ORC PLB IDM 2007/train_graphs/"
path2 = "ORC PLB IDM 2007/test_graphs/"
...
def listprep(name1, name2):
    ...
    np.save("ORC PLB/train_BindingAffinity.npy", train_BindingAffinity)
    np.save("ORC PLB/test_BindingAffinity.npy", test_BindingAffinity)
...
def create_test(threshold, cutoff, num_of_gaps):
    test_list = np.load("ORC PLB/test_list.npz", allow_pickle=True)
    for i in range(a, b+1):
        pdbid = test_list["arr_0"][i]
        print("Processing: ", pdbid)
        data = np.load("ORC PLB/Complexes/"+pdbid+"_complex_"+str(cutoff)+".npz", allow_pickle=True)
        ...
        np.save("ORC PLB/test_graphs/"+pdbid+"_"+str(cutoff)+"_"+str(threshold)+".npy", X_graphs)
...
def create_train(threshold, cutoff, num_of_gaps):
    ...
    train_list = np.load("ORC PLB/train_list.npz", allow_pickle=True)
    for i in range(a, b+1):
        ...
        for i in range(0, len(pro_coords)):
            for j in range(0, len(lig_coords)):
                ...
                X_graphs.append(temp_feature)
        #feature_x.append(X_feature)
        np.save("ORC PLB/train_graphs/"+pdbid+"_"+str(cutoff)+"_"+str(threshold)+".npy", X_graphs)

def gbt_rf(threshold, cutoff):
    X_train = np.load("ORC PLB/train_x_"+str(cutoff)+"_"+str(threshold)+".npy", allow_pickle=True)
    X_test = np.load("ORC PLB/test_x_"+str(cutoff)+"_"+str(threshold)+".npy", allow_pickle=True)
    Y_test = np.load("ORC PLB/test_BindingAffinity.npy", allow_pickle=True)
    Y_train = np.load("ORC PLB/train_BindingAffinity.npy", allow_pickle=True)
...
def convertall():
    ...
    for i in range(len(list2)):
        #convertpdb("ORC PLB/PDB/"+refined_set[i]+"_protein.pdb")
        #convertmol2("ORC PLB/MOL/"+refined_set[i]+"_ligand.mol2")
...
for num_of_bins in range(10, 101, 10):
    for fp in range(6, 16):
        train_set = np.load("ORC PLB IDM 2007/train_set_"+str(num_of_bins)+"_"+str(fp)+".npy", allow_pickle=True)
        temp = np.load("ORC PLB IDM 2007/train_graphs/1w8l_10_15.npy", allow_pickle=True)
        ...
        for i in range(len(temp)):
            ...
            for j in range(0, 10*fp):
                ...
                for v in temp[i][j].nodes():
                    try:
                        vertices[v] = temp[i][j].nodes[v]["ricciCurvature"]
                    except:
                        vertices[v] = 0
        ...
        X_train = np.concatenate((train_set[:509], [train_features], train_set[510:]))
        np.save("ORC PLB IDM 2007/train_set_"+str(num_of_bins)+"_"+str(fp)+".npy", X_train)
